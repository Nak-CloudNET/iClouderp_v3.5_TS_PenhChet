                    ...
                    <?php
                        if ($this->input->post('print')) {
                            if (!empty($barcodes)) {
                                ...
                                }elseif ($style == 65) { // Chea Kheng Barcode
                                ...
                                foreach ($barcodes as $item) {
                                    ...
                                    for ($r = 1; $r <= $item['quantity']; $r++) {
                                        ...
                                        if($style == 6) {
                                            ...
                                            if($item['price']) {
                                                echo '<span class="barcode_price" style="font-weight:bold; font-size:14px; display:block; padding-top:2px !important;margin-bottom:9px;">' . lang('$') . ' ';
                                                if($item['currencies']) {
                                                    foreach ($currencies as $currency) {
                                                        echo $currency->code . ': ' . $this->erp->formatMoney($item['price'] * $currency->rate).', ';
                                                    }
    ...
    function loadItems () {
        ...
        if (__getItem('bcitems')) {
            ...
            $.each(bcitems, function () {
                ...
                tr_html = '<td><input name="product[]" type="hidden" value="' + item_id + '"><span id="name_' + row_no + '">' + item.name + ' (' + item.code + ')</span><input type="hidden" class="count" value="' + item_id + '"><input type="hidden" class="rid" value="' + item.pro_id + '"></td>';
