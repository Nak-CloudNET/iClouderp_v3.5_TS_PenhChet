...
$(document).on('click', '.podel', function () {
    ...
    $(document).on('click', '.edit', function () {
        ...
        $('#net_cost').text(formatDecimal(net_cost));
    ...
    $(document).on('change', '#pcost, #ptax,#pdiscount, #pquantity', function () {
        ...
        $('#net_cost').text(formatDecimal(unit_cost));
        $('#pro_tax').text(formatDecimal(pr_tax_val));
		//checkDeposit();
    });
...
function loadItems() {
    if (__getItem('poitems')) {
        ...
        $.each(poitems, function () {
            ...
            item_cost = item_tax_method == 0 ? (net_unit_cost-pr_tax_val) : (unit_cost);
			
			item_costs = item_tax_method == 0 ? (net_unit_cost-pr_tax_val) : (net_unit_cost);
			...
			var stock_in_hand = (item.row.quantity);
            ...
            }else if(owner==null && admin==null && purchase_order_price==null){
			...
			if(owner || admin || purchase_order_cost){
				tr_html += '<td class="text-right"><input class="form-control text-center sp" name="serial[]" type="hidden" value="' + serial_no + '"><input class="form-control number_only text-center rcost" name="net_cost[]" type="text" id="cost_' + row_no + '" value="' + formatDecimal(net_unit_cost) + '"></td>';
            }else if(owner==null && admin==null && purchase_order_price==null){
                tr_html += '<td class="text-right" style="display: none"><input class="form-control text-center sp" name="serial[]" type="hidden" value="' + serial_no + '"><input class="form-control number_only text-center rcost" name="net_cost[]" type="text" id="cost_' + row_no + '" value="' + formatDecimal(net_unit_cost) + '"></td>';
            }
