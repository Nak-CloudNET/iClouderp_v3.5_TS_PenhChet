...
if (slwarehouse = __getItem('slwarehouse')) {
	...
	$(document).on('change','#pprice_show,#pprice, #ptax, #pdiscount,#pquantity', function () {
		...
		net_unit_price = unit_price - item_discount;
	    ...
	    $('#net_price').text(formatMoney(net_unit_price));
	    $('#pro_tax').text(formatMoney(pr_tax_val));
	    $('#pprice').val(formatMoney(unit_price));
	});
...
function loadItems() {
	if (__getItem('slitems')) {
		...
		var no=1;	
		if(__getItem('storage_details')){
			var product_dt = __getItem('storage_details');
			var product_details = product_dt.split('_');
			
			$.each(slitems, function () {
				var item = this;
				if(item.row.id == product_details[0]){
					item.row.details = product_details[1];
				}
			});
		}
		$.each(slitems, function () {
