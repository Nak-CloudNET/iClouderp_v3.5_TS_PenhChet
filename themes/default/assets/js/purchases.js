	...
	*/
     ...
     * Edit Row Quantity Method
     ...
     $(document).on("focus", '.rquantity, .received', function () {
        ...
        var new_qty = parseFloat(row.find('.rquantity').val()), new_rqty = parseFloat(((row.find('.received').val()))); //!= NaN || row.find('.received').val())? row.find('.received').val():0)),
        item_id 						= row.attr('data-item-id');
...
function loadItems() {
    if (__getItem('poitems')) {
        ...
        $.each(poitems, function () {
			...
			//var qty_received 	= (item.row.received)? item.row.received : item.row.qty;
			...
			tr_html += '<td class="text-right"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + formatMoney( (parseFloat(item_cost) + parseFloat(pr_tax_val)) * ((qty_received == item_qty? item_qty:qty_received)-0) ) + '</span></td>';
