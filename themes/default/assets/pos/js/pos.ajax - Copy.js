...
$(document).ready(function () {
	...
	$(document).on('click', '.edit', function () {
		...
		console.log(JSON.stringify(item.group_prices));
		if(item.group_prices !== false && item.group_prices) {
			var gp = 1;
			opt_group_price = $("<select id=\"pgroup_price\" name=\"pgroup_price\" class=\"form-control select\" />");
			$.each(item.group_prices, function () {
				if(gp == 1) {
					$("<option />", {value: 0, text: 'Default Price' + ' (' + formatDecimal(default_price) + ' USD)'}).appendTo(opt_group_price);
					if(item.row.price_id == 0) {
						$("<option />", {value: this.id, text: this.group_name + ' (' + formatDecimal(this.price) + ' '+ this.currency_code +')'}).attr("data-currency_code", this.currency_code).attr("data-unit-id", this.unit_id).appendTo(opt_group_price);
					}else{
						$("<option />", {value: this.id, text: this.group_name + ' (' + formatDecimal(this.price) + ' '+ this.currency_code +')'}).attr("data-currency_code", this.currency_code).attr("data-unit-id", this.unit_id).appendTo(opt_group_price);
					}
				}else{
	...
	$(document).on('click', '#editItem', function () {
		...
		if(positems[item_id].group_prices){
			...
			positems[item_id].group_prices[0].id = $('#pgroup_price').val() ? $('#pgroup_price').val() : '';
		}
		__setItem('positems', JSON.stringify(positems));
	 ...
	 * Product option change
	 ...
	 $(document).on('change', '#poption', function () {
		...
		var current_option_id = $(this).val();
	 	if(item.options !== false) {
	 		$.each(item.options, function () {
	 			if(current_price_group == 0 || !item.group_prices){
			...
			if(item.group_prices !== false){
				if(current_price_group > 0){
					var new_group_price_id = $("#pgroup_price option[data-unit-id='"+current_option_id+"']").val();
					$('#pgroup_price option').attr('disabled', false);
					$.each($('#pgroup_price option').not(':first-child'), function () {
						var val = $(this).val();
						if(val != new_group_price_id){
	 ...
	 * Product Group Price change
	 ...
	 $(document).on('change', '#pgroup_price', function () {
		...
		if(item.group_prices !== false && current_val > 0) {
			$.each(item.group_prices, function () {
				if(opt == 0){
