        ...
        if ($this->form_validation->run() == true){
            ...
            if (!$suspend) {
                ...
                for ($r = 0; $r < $p; $r++) {
                    ...
                    $residuals  = $g_total - $this->erp->formatDecimal($paid);
                    if (($g_total - $pos_b) > 0) {
    ...
    function ajaxproducts()
    {
        ...
        if ( ! empty($products)) {
            foreach ($products as $product) {
                ...
                $prods .= "<button id=\"product-" . $category_id . $count . "\" type=\"button\" value='" . $product->code . "' title=\"" . $product->name . "\" class=\"btn-prni btn-" . $this->pos_settings->product_button_color . " product pos-tip\" data-container=\"body\"><img src=\"" . base_url() . "assets/uploads/thumbs/" . $product->image . "\" alt=\"" . $product->name .'<br>' . "\" style='width:" . $this->Settings->twidth . "px;height:" . $this->Settings->theight . "px;' class='img-rounded' /><p style='margin: 0'>" . character_limiter($product->code, 40) . "</p><p style='margin: 0'>". character_limiter($product->name, 40) ."</p></button>";
                $pro++;
            }
        }
        $prods .= "</div>";
