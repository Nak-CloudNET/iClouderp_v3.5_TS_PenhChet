...
if (!function_exists('getAvgCost')) {
    ...
    {   //return false;
        ...
        $q_all_purchase_stock_trans = $ci->db->query("SELECT  st.product_id,
                                    ...
                                    st.tran_type IN ('PURCHASE', 'OPENING QUANTITY', 'ADJUSTMENT')  GROUP BY st.product_id ");
    ...
    function optimizeSale($tran_date)
    {
        ...
        if ($q_all_sales->num_rows() > 0) {
            foreach ($q_all_sales->result() as $row_all_sale) {
                ...
                $q_delivery = $ci->db->query("SELECT id FROM erp_deliveries
                            WHERE issued_sale_id = {$row_all_sale->id}
                            AND delivery_status = 'completed'
                              AND `type` = 'sale_order'
                            ");
                if ($q_delivery->num_rows() == 0) {
